...
def main():
    ...
    # Create GitHub repo, clone it, create a virtual environment, install dependencies, and create a main.py file.

    # Milestone 2: API Data Retrieval - Code function fetch_311_requests() to retrieve data from the 311 API
    # and return it as a list of dictionaries. Use the requests library to make the API call,
   ...
   '''
    ...
    # Milestone 3: Data Cleaning with Pandas - Moved to function load_data()
    df = load_data()

    # Milestone 4: The Visual Dashboard
    ...
    st.bar_chart(filtered.groupby(filtered["open_date_time"].dt.hour)["issue_type"].size())

    # Milestone 4 - Step 3: Layer on More Features
    #street = st.selectbox("Choose a street", filtered["street_name"].unique())
    #filtered = filtered[filtered["street_name"] == street]
    #st.metric("Total Requests for this issue type", len(filtered))
    #st.line_chart(filtered.groupby(filtered["open_date_time"].dt.hour)["issue_type"].size())
    #st.bar_chart(filtered.groupby(filtered["open_date_time"].dt.hour)["issue_type"].size())
...
def load_data(limit=1000):
    ...
    # (single fetch — reused for both the emptiness check and the DataFrame build)
    ...
    # Milestone 3, Step 2: Clean deliberately - narrate why for each line, not just how
    # ---------- Clean deliberately ----------
    ...
    # Milestone 3, Step 3: Answer real questions with groupby ----------
